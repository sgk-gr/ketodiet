  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20 selection:bg-emerald-100 selection:text-emerald-900">
        ...
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <div>
            <div className="flex items-center space-x-2">
              <h1 className="text-base font-bold text-slate-900 tracking-tight">Spiros Low-Carb 16:8</h1>
      ...
      <main className="max-w-4xl mx-auto px-4 py-4 sm:py-6 space-y-4 sm:space-y-6">
        ...
        {/* 2. TIME-AWARE ADVISOR BOX (LIGHT THEME) */}
          ...
          {timePhase === 'morning_fast' && (
            <div className="space-y-2.5 text-xs">
              <div className="p-3 rounded-xl bg-white border border-slate-200 space-y-1 shadow-sm">
                <span className="font-bold text-sky-800 uppercase block text-[11px]">
                  Τι επιτρέπεται τώρα:
                </span>
                <p className="text-slate-800">
                  500ml δροσερό νερό, σκέτος καφές (ελληνικός/espresso) ή πράσινο τσάι χωρίς ζάχαρη.
                </p>
          ...
          {timePhase === 'snack_now' && (
            <div className="space-y-2.5 text-xs">
              <div className="p-3.5 rounded-xl bg-white border border-teal-300 shadow-sm space-y-1.5">
                <span className="font-bold text-teal-800 uppercase text-[11px] block">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div className="rounded-xl p-3.5 border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center justify-between text-xs text-slate-500 mb-0.5">
              <span className="font-semibold uppercase tracking-wider text-slate-700">Βάρος Σήμερα</span>
              <span className="text-[11px]">Αρχικό: 105kg</span>
            </div>
            <div className="flex items-baseline space-x-1.5 my-0.5">
              <span className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">{latestWeight.toFixed(1)}</span>
            ...
            <p className="text-[11px] text-slate-500 pt-1 border-t border-slate-100 flex justify-between">
          ...
          <div className="rounded-xl p-3.5 border border-slate-200 bg-white shadow-sm">
            ...
            <div className="flex items-baseline space-x-1.5 my-0.5">
              <span className="text-2xl sm:text-3xl font-extrabold text-indigo-700 tracking-tight">-{spineRelief}</span>
          ...
          <div className="rounded-xl p-3.5 border border-slate-200 bg-white shadow-sm">
            ...
            <div className="my-0.5">
              ...
              <p className="text-[11px] text-slate-500">20:00 - 12:00 (Νηστεία)</p>
            </div>
        ...
        <div className="rounded-xl sm:rounded-2xl p-4 border border-slate-200 bg-white shadow-sm space-y-3">
            ...
            {(!isLunchPast || showPastMeals || !isSelectedDayToday) ? (
              lunchMeal && (
                <div className={`p-3.5 rounded-xl border space-y-1.5 ${
                  ...
                  <p className="text-slate-600 leading-relaxed text-[11px]">{lunchMeal.description}</p>
            ...
            {(!isSnackPast || showPastMeals || !isSelectedDayToday) ? (
              snackMeal && (
                <div className={`p-3.5 rounded-xl border space-y-1.5 ${
                  ...
                  <p className="text-slate-600 leading-relaxed text-[11px]">{snackMeal.description}</p>
            ...
            {(!isDinnerPast || showPastMeals || !isSelectedDayToday) ? (
              dinnerMeal && (
                <div className={`p-3.5 rounded-xl border space-y-1.5 ${
                  ...
                  <p className="text-slate-600 leading-relaxed text-[11px]">{dinnerMeal.description}</p>
                  <p className="text-[11px] text-indigo-800 font-semibold pt-1">
          ...
          {isSelectedDayToday && (isLunchPast || isSnackPast || isDinnerPast) && (
            <div className="pt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-[11px] text-slate-500 border-t border-slate-100">
              <span>
                {isDinnerPast 
        ...
        {/* 6. ΟΙ 2 ΣΤΗΛΕΣ: ΤΙ ΤΡΩΣ & ΤΙ ΔΕΝ ΤΡΩΣ (LIGHT) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="rounded-xl p-4 border border-emerald-200 bg-emerald-50/40 space-y-3">
            <div className="border-b border-emerald-200 pb-2">
              ...
              <p className="text-[11px] text-slate-500">Καθαρή πρωτεΐνη, λαχανικά, καλά λιπαρά</p>
            </div>
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="p-2.5 rounded-lg bg-white border border-emerald-100 shadow-sm">
                ...
                <p className="text-slate-700 leading-relaxed text-[11px]">
                  Έξτρα Παρθένο Ελαιόλαδο, Αβοκάντο, Στραγγιστό Γιαούρτι 2%, Αμύγδαλα & Καρύδια.
                </p>
              </div>
              ...
              <div className="p-2.5 rounded-lg bg-white border border-emerald-100 shadow-sm">
                ...
                <p className="text-slate-700 leading-relaxed text-[11px]">
                  Νερό (3L), Σκέτος Καφές, Πράσινο Τσάι, Σόδα.
                </p>
              </div>
            </div>
            ...
            {searchFood && allowedFoods.length > 0 && (
              <div className="pt-2 border-t border-emerald-200 space-y-1.5">
                <span className="text-[11px] font-bold text-emerald-800">Επιτρεπόμενα:</span>
                {allowedFoods.map(f => (
                  <div key={f.id} className="p-2 rounded bg-white border border-emerald-300 text-[11px] shadow-sm">
          ...
          <div className="rounded-xl p-4 border border-rose-200 bg-rose-50/40 space-y-3">
            <div className="border-b border-rose-200 pb-2">
              <h2 className="text-sm sm:text-base font-bold text-rose-800 uppercase tracking-wide">
                Τι Κόβεις (Απαγορεύονται)
              </h2>
              <p className="text-[11px] text-slate-500">Μπλοκάρουν το λίπος & προκαλούν φλεγμονή στη μέση</p>
            </div>
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="p-2.5 rounded-lg bg-white border border-rose-100 shadow-sm">
                ...
                <p className="text-slate-700 leading-relaxed text-[11px]">
                  Σοκολάτες, Πάστες, Παγωτά, Μπισκότα, Μέλι, Μαρμελάδες, Δημητριακά.
                </p>
              </div>
              ...
              <div className="p-2.5 rounded-lg bg-white border border-rose-100 shadow-sm">
                ...
                <p className="text-slate-700 leading-relaxed text-[11px]">
                  Αναψυκτικά με ζάχαρη, Χυμοί, Μπύρα, Γλυκά ποτά.
                </p>
              </div>
            </div>
            ...
            {searchFood && forbiddenFoods.length > 0 && (
              <div className="pt-2 border-t border-rose-200 space-y-1.5">
                <span className="text-[11px] font-bold text-rose-800">Απαγορευμένα:</span>
                {forbiddenFoods.map(f => (
                  <div key={f.id} className="p-2 rounded bg-white border border-rose-300 text-[11px] shadow-sm">
        ...
        <div className="p-3.5 sm:p-4 rounded-xl border border-indigo-200 bg-indigo-50/70 text-xs">
          ...
          <p className="text-slate-700 leading-relaxed text-[11px] sm:text-xs">
            <strong>Απαγορεύεται το τρέξιμο & οι κραδασμοί.</strong> Επιτρέπεται μόνο <strong>στατικό ποδήλατο με πλάτη</strong>, <strong>περπάτημα σε ίσιωμα (15-20')</strong> και <strong>κολύμβηση</strong>. Πίνετε <strong>3 λίτρα νερό</strong> την ημέρα.
          </p>
        </div>

        {/* 8. ΚΑΤΑΧΩΡΗΣΗ ΚΙΛΩΝ (LIGHT) */}
        <div className="rounded-xl p-4 border border-slate-200 bg-white shadow-sm">
          ...
          <form onSubmit={handleSaveWeight} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            <input
              ...
              className="bg-slate-50 border border-slate-300 rounded-lg px-3.5 py-2 text-sm text-slate-900 font-bold focus:outline-none focus:border-emerald-600 focus:bg-white w-full sm:w-36 text-center sm:text-left"
            />
            <button
              type="submit"
          ...
          {weightLogs.length > 0 && (
            <div className="mt-3 pt-2 border-t border-slate-100 flex flex-wrap gap-1.5 text-xs">
              <span className="text-slate-500 font-semibold self-center mr-1 text-[11px]">Ιστορικό:</span>
              {[...weightLogs].reverse().slice(0, 5).map((log) => (
                <div key={log.id} className="flex items-center space-x-1 px-2 py-0.5 rounded bg-slate-100 border border-slate-200 text-slate-700 text-[11px]">
